...
public:
    ...
    // only a rotating chrome dome sprite and buttons/combos become invisible
    // hit areas (componentID "hit") whose lit states the editor masks from
    // the ON plate.
    bool plate = false;
    ...
    //==========================================================================
    // Flat dropdown inside a clean white pill: pink text, small pink chevron.
    void drawComboBox (juce::Graphics& g, int width, int height, bool,
                       int, int, int, int, juce::ComboBox& box) override
    {
        if (plate && box.getComponentID() == "hit") return;

        auto r = juce::Rectangle<float> (0.0f, 0.0f, (float) width, (float) height).reduced (1.5f);
        paintPill (g, r, r.getHeight() * 0.5f, false, false);
    ...
    {
        label.setBounds (4, 0, box.getWidth() - 26, box.getHeight());
