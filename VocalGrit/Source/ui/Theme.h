...
namespace theme
{
   ...
   #if VG_HAS_BUNDLED_FONT
    // When a font file is bundled (dropped into <repo>/fonts/ and embedded by
    // CMake), it's loaded once and reused so the UI renders identically on
    // Windows and macOS without depending on system-installed fonts.
    ...
    // Minimal monochrome line icons for module headers. Drawn within `area`.
    inline void moduleIcon (juce::Graphics& g, const juce::String& kind,
                            juce::Rectangle<float> area, juce::Colour colour)
    {
        g.setColour (colour);
        const auto c = area.getCentre();
        const float s = juce::jmin (area.getWidth(), area.getHeight());
        const float lw = juce::jmax (1.2f, s * 0.085f);

        if (kind == "doubler")
        {
            const float r = s * 0.26f;
            g.drawEllipse (c.x - r * 1.5f, c.y - r, r * 2.0f, r * 2.0f, lw);
            g.drawEllipse (c.x - r * 0.5f, c.y - r, r * 2.0f, r * 2.0f, lw);
        }
        else if (kind == "delay")
        {
            for (int i = 0; i < 3; ++i)
            {
                const float a = 1.0f - (float) i * 0.32f;
                const float h = s * (0.5f - (float) i * 0.1f);
                const float x = c.x - s * 0.32f + (float) i * s * 0.28f;
                g.setColour (colour.withAlpha (a));
                g.fillRoundedRectangle (x, c.y - h * 0.5f, lw * 1.4f, h, lw * 0.6f);
            }
        }
        else if (kind == "reverb")
        {
            for (int i = 1; i <= 3; ++i)
            {
                const float r = s * 0.12f * (float) i;
                juce::Path p;
                p.addCentredArc (c.x - s * 0.12f, c.y, r, r, 0.0f,
                                 -juce::MathConstants<float>::halfPi * 0.9f,
                                  juce::MathConstants<float>::halfPi * 0.9f, true);
                g.setColour (colour.withAlpha (1.0f - (float) (i - 1) * 0.25f));
                g.strokePath (p, juce::PathStrokeType (lw));
            }
        }
        else if (kind == "glitch")
        {
            juce::Path p;
            const float x0 = c.x - s * 0.34f, x1 = c.x + s * 0.34f;
            const float yT = c.y - s * 0.2f, yB = c.y + s * 0.2f;
            p.startNewSubPath (x0, c.y);
            p.lineTo (x0 + s * 0.17f, c.y);
            p.lineTo (x0 + s * 0.17f, yT);
            p.lineTo (x0 + s * 0.34f, yT);
            p.lineTo (x0 + s * 0.34f, yB);
            p.lineTo (x0 + s * 0.51f, yB);
            p.lineTo (x0 + s * 0.51f, c.y);
            p.lineTo (x1, c.y);
            g.strokePath (p, juce::PathStrokeType (lw, juce::PathStrokeType::mitered,
