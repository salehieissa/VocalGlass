...
void VocalQEditor::timerCallback()
{
    ...
    if (chassisImg.isValid())
    {
        ...
        {
            const double v = knobs[i]->getSlider().getValue();
            if (v != shownKnob[i])
            {
                shownKnob[i] = v;
                repaint (plateFracRect (knobCx[i] - ringMaxR, knobCy - ringMaxR * ar,
        ...
        auto tabRect = [&] (int b)
        {
            return plateFracRect (tabX[b], tabY0, tabX[b + 1] - tabX[b], tabY1 - tabY0)
                       .expanded (2);
        };
        if (selectedBand != shownBand)
        {
            if (shownBand >= 0) repaint (tabRect (shownBand));
            shownBand = selectedBand;
            repaint (tabRect (selectedBand));
        }

        if (powerBtn.getToggleState() != shownPower)
        {
            shownPower = powerBtn.getToggleState();
            repaint (plateFracRect (powX0, powY0, powX1 - powX0, powY1 - powY0).expanded (2));
        }
        if (chan != shownChan)
        {
            shownChan = chan;
            repaint (plateFracRect (chanX0, chanY[0], chanX1 - chanX0,
                                    chanY[3] - chanY[0]).expanded (2));
        }
        if (type != shownType)
        {
            shownType = type;
            repaint (plateFracRect (typeX0, typeY[0], typeX1 - typeX0,
                                    typeY[6] - typeY[0]).expanded (2));
        }
        const bool solo = soloBtn.getToggleState();
        if (solo != shownSolo)
        {
            shownSolo = solo;
            repaint (plateFracRect (soloX0, soloY0, soloX1 - soloX0, soloY1 - soloY0).expanded (2));
        }
    }
}
...
void VocalQEditor::resized()
{
    ...
    if (chassisImg.isValid() && chassisOnImg.isValid())
    {
        ...
        // (size fractions are of the full canvas WIDTH, so scale by iw * sx)
        const float domeSide = knobDomeDia * iw * sx / 0.78f;   // matches domeScale
        Knob* knobs[7] = { &freqK, &qK, &gainK, &rangeK, &threshK, &atkK, &relK };
        for (int i = 0; i < 7; ++i)
        {
            const float cx = (knobCx[i] * iw - (float) plateCrop.getX()) * sx;
            const float cy = (knobCy    * ih - (float) plateCrop.getY()) * sy;
