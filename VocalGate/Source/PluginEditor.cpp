...
void VocalGateEditor::timerCallback()
{
    ...
    // (Value labels, the preset label and the GR meter are child components
    // that repaint themselves; only ring wedges + pills live in paintPlate.)
    using namespace plategeo;
    struct KnobRegion { juce::Slider* k; float cx, cy, maxR; };
    const KnobRegion regions[] = {
        { &threshKnob,  thrCx, thrCy, thrMaxR },
        { &rangeKnob,   rngCx, rngCy, rngMaxR },
        { &attackKnob,  atkCx, atkCy, envMaxR },
        { &holdKnob,    hldCx, hldCy, envMaxR },
        { &releaseKnob, relCx, relCy, envMaxR },
        { &hystKnob,    hysCx, hysCy, envMaxR },
        { &hpfKnob,     hpfCx, hpfCy, hpfMaxR },
    };
    ...
    const bool sc = scListenBtn.getToggleState();
    if (sc != shownScListen)
    {
        shownScListen = sc;
        repaint (plateFracRect (scX0, scY0, scX1, scY1).expanded (12));
    }
}
